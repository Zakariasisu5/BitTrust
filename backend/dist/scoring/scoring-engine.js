 ...
 */
function scoreWalletAge(ageDays) {
    if (ageDays <= 0)
        return 0;
    if (ageDays < 30)
        return Math.round((ageDays / 30) * 5);
    if (ageDays < 180)
        return 5 + Math.round(((ageDays - 30) / 150) * 7);
    if (ageDays < 365)
        return 12 + Math.round(((ageDays - 180) / 185) * 5);
 ...
 */
function scoreTxQuality(totalTx, successfulTx, failedTx) {
    if (totalTx === 0)
        return 0;
 ...
 */
function scoreDefiActivity(totalTx, contractCallCount, hasDefiActivity, uniqueContracts) {
    if (totalTx === 0)
        return 0;
 ...
 */
function scoreCommunityEngagement(stxBalance, walletAgeDays, totalTx) {
    // Balance tier (in STX: 1 STX = 1_000_000 microSTX)
    const stx = Number(stxBalance) / 1000000;
    let balancePts = 0;
    if (stx >= 10000)
        balancePts = 10;
    else if (stx >= 1000)
...
function getTier(score) {
    if (score >= 81)
        return { tier: "A+", tierLabel: "Highly Trusted" };
    if (score >= 61)
        return { tier: "A", tierLabel: "Trusted / Low Risk" };
    if (score >= 31)
        return { tier: "B", tierLabel: "Medium Risk" };
    return { tier: "C", tierLabel: "High Risk" };
}
// ── Explanation generator ─────────────────────────────────────────────────────
function generateExplanation(score, activity, breakdown) {
    const parts = [];
...
// ── Main export ───────────────────────────────────────────────────────────────
function calculateReputationScore(activity) {
    const walletAge = scoreWalletAge(activity.walletAgeDays);
    const txQuality = scoreTxQuality(activity.totalTxCount, activity.successfulTxCount, activity.failedTxCount);
    const defiActivity = scoreDefiActivity(activity.totalTxCount, activity.contractCallCount, activity.hasDefiActivity, activity.uniqueContractsInteracted);
    const communityEngagement = scoreCommunityEngagement(activity.stxBalance, activity.walletAgeDays, activity.totalTxCount);
    ...
    const factors = [
        ...
        },
        {
            ...
            raw: `${(Number(activity.stxBalance) / 1000000).toFixed(2)} STX balance`,
